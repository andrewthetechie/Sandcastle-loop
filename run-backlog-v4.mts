...
//
// v4 vs v3 — the coder escalation ladder. v3 ran every review round on one
// model and capped at 10 rounds. Measured over 451 v3 tasks, the probability a
// task needs another round *rises* with the round number (~44% after round 1,
// ~65% through rounds 3-6, 85-92% past round 7), and rounds 7-10 burned ~20h of
// wall-clock on tasks that overwhelmingly never converged. v4 spends that
// budget on capability instead of repetition:
//
//   rounds 1-2  models.rework       (cheap/local tier 1)
//   rounds 3-4  models.reworkTier2
//   rounds 5-6  models.reworkTier3
//   round 7+    does not exist — maxReviewRounds is 6
//
// Thresholds are configurable (`coderEscalation` in .sandcastle/config.mts, or
// --escalate-tier2-round / --escalate-tier3-round). Round 1 always runs the
// fresh-start coder on `models.coder`.
//
// Crucially, the engine clears its no-progress fingerprint history whenever the
// model changes. `failedRoundRepeatLimit` encodes "same model, same input, same
// failure, therefore pointless"; a stronger model invalidates that inference.
// Without the reset a task whose cheap model fails identically on rounds 1-3
// would be killed by the repeat limit at exactly the round the ladder was meant
// to rescue it. See `agentModelForRound` in per-branch-engine.mts.
//
// Review is unchanged from v3: same reviewer model, same single-verdict gate.
//
...
//
// Usage:
//   tsx run-backlog-v4.mts --label <name[,name2]> [--base-branch main]
//     [--idle-timeout 1800] [--model-coder <m>] [--model-rework <m>]
//     [--model-rework-tier2 <m>] [--model-rework-tier3 <m>]
//     [--escalate-tier2-round 3] [--escalate-tier3-round 5]
...
import { BACKLOG_V4_ENGINE_POLICY } from "./per-branch-policy.mts";
import {
...
// Loop bounds
const MAX_REVIEW_ROUNDS = BACKLOG_V4_ENGINE_POLICY.maxReviewRounds;
const FAILED_ROUND_REPEAT_LIMIT =
  BACKLOG_V4_ENGINE_POLICY.failedRoundRepeatLimit;
const MAX_ITERATIONS = 250; // outer-loop safety backstop (re-run to continue)
const CODER_MAX_ITERATIONS = BACKLOG_V4_ENGINE_POLICY.coderMaxIterations;
const MAX_RECOVERY_ATTEMPTS = BACKLOG_V4_ENGINE_POLICY.maxRecoveryAttempts;
const MAX_PUSH_RECOVERY_ATTEMPTS = 2; // remote issue-branch push recovery
const BACKLOG_V4_SHARED_ENGINE_POLICY = {
  ...BACKLOG_V4_ENGINE_POLICY,
...
const USAGE = [
  "Usage: tsx run-backlog-v4.mts --label <name[,name2]> [--base-branch <name>]",
  "  [--idle-timeout <seconds>] [--model-coder <model>] [--model-rework <model>]",
  "  [--model-rework-tier2 <model>] [--model-rework-tier3 <model>]",
  "  [--escalate-tier2-round <n>] [--escalate-tier3-round <n>]",
].join("\n");
...
const modelReworkTier2Override = readStringFlag("--model-rework-tier2");
const modelReworkTier3Override = readStringFlag("--model-rework-tier3");
...
const REWORK_TIER2_MODEL =
  modelReworkTier2Override ?? LOOP_CONFIG.models.reworkTier2;
const REWORK_TIER3_MODEL =
  modelReworkTier3Override ?? LOOP_CONFIG.models.reworkTier3;

function readRoundFlag(flag: string, fallback: number): number {
  const raw = readStringFlag(flag);
  if (raw === undefined) return fallback;
  const parsed = Number.parseInt(raw, 10);
  if (!Number.isInteger(parsed) || parsed < 2) {
    throw new Error(`${flag} must be an integer >= 2, got ${raw}`);
  }
  return parsed;
}

// CLI flags override config, which overrides the built-in defaults (3 and 5).
const CODER_ESCALATION_LADDER: CoderEscalationLadder = {
  tier1Model: REWORK_MODEL,
  tier2Model: REWORK_TIER2_MODEL,
  tier3Model: REWORK_TIER3_MODEL,
  tier2FromRound: readRoundFlag(
    "--escalate-tier2-round",
    LOOP_CONFIG.coderEscalation.tier2FromRound,
  ),
  tier3FromRound: readRoundFlag(
    "--escalate-tier3-round",
    LOOP_CONFIG.coderEscalation.tier3FromRound,
  ),
};
validateCoderEscalationLadder(CODER_ESCALATION_LADDER);

// The model a given round will actually run. Round 1 (and any round the engine
// decides is not rework) runs the fresh-start coder; every rework round takes
// its model from the ladder. This is the single source of truth for both the
// agent invocation and the engine's model-change fingerprint reset, so the two
// can never disagree about which model ran.
function resolveAgentModelForRound(round: number, isRework: boolean): string {
  return isRework
    ? coderTierModelIdentity(CODER_ESCALATION_LADDER, round)
    : CODER_MODEL;
}
...
console.log(
  [
    ...
    `models=coder:${CODER_MODEL},reviewer:${REVIEWER_MODEL}`,
    `reviewerMaxAttempts=${LOOP_CONFIG.reviewer.maxAttempts}`,
...
console.log(
  describeCoderEscalationLadder(CODER_ESCALATION_LADDER, MAX_REVIEW_ROUNDS),
);
for (const warning of coderEscalationLadderWarnings(
  CODER_ESCALATION_LADDER,
  MAX_REVIEW_ROUNDS,
)) {
...
// v3's sameCoderReworkModelWarning lived here. It warned when CODER_MODEL and
// REWORK_MODEL matched, because that made rework prompt-only escalation. Under
// the v4 ladder that condition is the *intended* configuration for rounds 1-2,
// so the warning would fire on every healthy run. Ladder coherence checks live
// in coderEscalationLadderWarnings instead.
...
async function runBacklogIssueViaSharedEngine(input: {
  ...
  try {
    ...
    const outcome = await runPerBranchEngine({
      task,
      policy: BACKLOG_V4_SHARED_ENGINE_POLICY,
      deps: {
        async createSandbox() {
          return {
        ...
        async invokeCoder(input) {
          ...
          if (isRework) {
            // Escalation ladder: pick the model for this round. The agent
            // definition file is rewritten per invocation, so a per-round model
            // needs no extra machinery — but it must be threaded through all
            // three of buildAgentDefinition, opencode() and the metrics record,
            // or the recorded `model` column stops matching what actually ran.
            const reworkTier = resolveCoderTier(
              CODER_ESCALATION_LADDER,
              input.round,
            );
            ...
            const reworkRunName = `rework #${issue.number} r${input.round} t${reworkTier.tier}`;
            ...
            try {
              runResult = await recordMeasuredAgentRun(
                ...
                () =>
                  sandbox.run({
                    name: reworkRunName,
                    agent: sandcastle.opencode(reworkTier.model, {
                      agent: REWORK_AGENT_CONFIG.name,
                    }),
        ...
        async acquireReviewer(input) {
          ...
          recordReviewerResult({
            ...
            maxAttempts: BACKLOG_V4_SHARED_ENGINE_POLICY.reviewerMaxAttempts,
            status:
        ...
        agentModelForRound({ round, isRework }) {
          return resolveAgentModelForRound(round, isRework);
        },
        onModelEscalation({ round, fromModel, toModel }) {
          console.log(
            `  escalating #${issue.number} at round ${round}: ${fromModel} -> ${toModel} (no-progress history reset)`,
          );
        },
      },
    });
