...
const SYSTEM_PROMPT_PATH = "./coder-agent-system-prompt-prd.md";
const USER_PROMPT_PATH = "./coder-user-prompt-prd.md";
...
const EXPECTED_SYSTEM_PROMPT = [
  ...
  "",
  "The repository's project instructions (AGENTS.md) in your context are binding: follow their documented conventions for types, shared helpers, structure, and naming in every edit. The reviewer blocks documented-standards violations.",
  "",
  ...
  "- Adding utilities, hooks, helpers, packages, or sub-agents unless the issue requires them or a clear sibling pattern calls for the same structure.",
  "",
  "Matching local structure is not scope creep: when adjacent code implements the same kind of thing through an extracted component, hook, or helper, mirror that structure (including creating the new file) instead of inlining a divergent copy. Reuse existing exports and generated types instead of re-deriving them. Scope discipline restricts which behavior you implement, not whether you match local structure.",
  "",
...
test("coder agent system prompt contains only the static coder instructions", () => {
  ...
  assert.match(systemPrompt, /# Missing Dependencies/);
  assert.doesNotMatch(systemPrompt, /\{\{/);
  assert.doesNotMatch(systemPrompt, /<prd>/);
});

test("coder user prompt contains only the slim issue template", () => {
