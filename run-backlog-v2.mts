...
const USAGE =
  "Usage: tsx run-backlog.mts --label <name[,name2]> [--base-branch <name>] [--idle-timeout <seconds>]";
...
function rebaseStaleIssueBranchBeforeCoder(
  ...
  console.log(
    `  ${issueBranch} is stale vs ${originBaseRef()}; rebasing before coder`,
  );
  ...
  try {
    for (let round = 1; round <= MAX_REVIEW_ROUNDS; round++) {
      ...
      if (round === 1) {
        ...
        if (!guard.ok) {
          terminalReason = "stuck_rebase_conflict";
          lastFeedback = guard.feedback;
          break;
        }
      }
