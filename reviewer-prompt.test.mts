...
const SYSTEM_PROMPT_PATH = "./reviewer-agent-system-prompt-prd.md";
const USER_PROMPT_PATH = "./reviewer-user-prompt-prd.md";
...
test("reviewer agent system prompt contains only the static reviewer instructions", () => {
  const systemPrompt = readFileSync(SYSTEM_PROMPT_PATH, "utf8");

  assert.equal(systemPrompt, EXPECTED_SYSTEM_PROMPT);
  assert.match(systemPrompt, /<review>/);
  assert.match(systemPrompt, /# Output Contract/);
  assert.match(systemPrompt, /# Hard Blocking Patterns/);
  assert.match(systemPrompt, /## Example: approved/);
  assert.match(systemPrompt, /## Example: changes_requested/);
  assert.match(systemPrompt, /## Example: needs_human_review/);
  assert.doesNotMatch(systemPrompt, /\{\{/);
  assert.doesNotMatch(systemPrompt, /!`/);
  assert.doesNotMatch(systemPrompt, /<prd>/);
  assert.doesNotMatch(systemPrompt, /<diff>/);
});

test("reviewer user prompt contains only the slim dynamic review template", () => {
  ...
  assert.match(userPrompt, /\{\{BASE_BRANCH\}\}/);
  assert.match(userPrompt, /\{\{REVIEW_BASE_SHA\}\}/);
  assert.match(userPrompt, /\{\{DIFF_BYTES\}\}/);
