...
const SYSTEM_PROMPT_PATH = "./code-quality-agent-system-prompt-prd.md";
const USER_PROMPT_PATH = "./code-quality-user-prompt-prd.md";
...
test("code-quality agent system prompt contains only the static quality-gate instructions", () => {
  const systemPrompt = readFileSync(SYSTEM_PROMPT_PATH, "utf8");

  assert.equal(systemPrompt, EXPECTED_SYSTEM_PROMPT);
  assert.match(systemPrompt, /extra_review/);
  assert.match(systemPrompt, /# Operating rules/);
  assert.match(systemPrompt, /# Review target/);
...
test("code-quality user prompt contains only the file-backed input template", () => {
